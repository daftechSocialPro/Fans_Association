...
import { urlDegafi, urlDegafiSetting, urlMahber } from 'src/endpoints'
...
export default function DegafiCreate({ user, setIsLodding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      axios
        .post(urlDegafi, formData)
        .then((res) => {
          setIsLodding(false)
          navigate('/Degafi')
          customToast('Degafi Successfully created', 0)
        })
        .catch((err) => {
          setIsLodding(false)
          alert(err)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader style={{ backgroundColor: '#232324de', color: '#e99313' }} className="">
            <CRow>
              <CCol sm={10}>
                <strong>Degafi</strong> <small>Create Profile</small>
              </CCol>
            </CRow>
          </CCardHeader>
