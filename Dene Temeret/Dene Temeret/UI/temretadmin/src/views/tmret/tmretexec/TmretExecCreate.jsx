...
import {   urlTmretExec } from '../../../endpoints'
...
export default function TmretExecCreate({ user,setIsLodding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      axios
        .post(urlTmretExec, formData)
        .then((res) => {
          setIsLodding(false)
          navigate('/tmretexec')
          customToast('Tmret Executives Successfully created', 0)
        })
        .catch((err) => {
          setIsLodding(false)
          alert(err)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader style={{ backgroundColor: '#232324de', color: '#e99313' }} className="">
            <CRow>
              <CCol sm={10}>
                <strong>Tmret Executives</strong> <small>Create Profile</small>
              </CCol>
            </CRow>
          </CCardHeader>
