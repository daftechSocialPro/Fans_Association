...
import { urlTmretExec, assetUrl } from 'src/endpoints'
function TmretExec({ user }) {
  const [tmret, setTmret] = useState([])
  const [visibleXL, setVisibleXL] = useState(false)
  const [tmretExec, settmretExec] = useState({})

  const naviagate = useNavigate()

  useEffect(() => {
    axios.get(urlTmretExec).then((res) =>{ setTmret(res.data)
    
  
  })
  }, [])

  const addTmretExec = (e) => {
    e.preventDefault()
    naviagate('/tmretexec/create')
  }
  ...
  return (
    <>
      <CModal size="xl" visible={visibleXL} onClose={() => setVisibleXL(false)}>
        ...
        >
          <CModalTitle>Tmret Executives Profile View </CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CCardBody>
            <MDBRow>
              <MDBCol lg="4">
                <MDBCard className="mb-4">
                  <MDBCardBody className="text-center">
                    <MDBCardImage
                      src={getImage(tmretExec.userPhoto)}
                      alt="avatar"
                    ...
                    <MDBRow>
                      ...
                      <MDBCol sm="9">
                        <MDBRow>
                          <MDBCol sm="6">
                            <MDBCardText className="text-muted">
                              {tmretExec && tmretExec.fromDate}
                            </MDBCardText>
                          </MDBCol>
                          <MDBCol sm="6">
                            <MDBCardText className="text-muted">
                              {tmretExec && tmretExec.toDate}
                            </MDBCardText>
                          </MDBCol>
                        </MDBRow>
                      </MDBCol>
              ...
              <MDBCol lg="12">
                <MDBCard className="mb-4">
                  <MDBCardBody>
                    <div dangerouslySetInnerHTML={{ __html: tmretExec.description }}></div>
                  </MDBCardBody>
                </MDBCard>
              </MDBCol>
            </MDBRow>
        ...
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader style={{ backgroundColor: '#232324de', color: '#e99313' }} className="">
              <CRow>
                <CCol sm={10}>
                  <strong>Tmret Executives</strong> <small>List</small>
                </CCol>
                <CCol sm={2} className="d-flex justify-content-end">
                  <CButton
                    className="text-right bg-white"
                    onClick={addTmretExec}
              ...
              <CTable align="middle" className="mb-0 border" hover responsive>
                <CTableHead color="light">
                  <CTableRow>
                    ...
                    <CTableHeaderCell>Tmret Executive</CTableHeaderCell>
